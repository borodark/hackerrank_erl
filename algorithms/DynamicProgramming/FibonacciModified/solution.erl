...
%%-----------------------------------------------------------------------------
fib_rec_naive(0) -> io:format("~p~n",[0]), 0;
fib_rec_naive(1) -> io:format("~p~n",[1]), 1;
...
fib_arith(0, F1, _F2) -> io:format("~p~n",[F1]), F1;
fib_arith(N, F1,  F2) -> Rc = fib_arith(N - 1, F2, F1 + F2), io:format("~p~n",[Rc]), Rc.
